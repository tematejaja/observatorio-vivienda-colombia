...
def indicador(tabla: pd.DataFrame, ciudad: str, anio: str, nombre_indicador: str) -> dict | None:
    ...
    return {
        ...
        "valor": valor,
        "error_estandar": float(r["error_estandar"]) if pd.notna(r["error_estandar"]) else None,
        "ic95_inf": float(r["ic95_inf"]) if pd.notna(r["ic95_inf"]) else None,
        "ic95_sup": float(r["ic95_sup"]) if pd.notna(r["ic95_sup"]) else None,
        "deff": float(r["deff"]) if pd.notna(r["deff"]) else None,
